...
public record Titular
{
    public string Dni {
        get;
        init => field = !IsDniValido(value) ? throw new ArgumentException("El Dni no es valido.") : value;
    } = string.Empty;

    public string Nombre {
        get; 
        init => field = !IsNombreValido(value) ? throw new ArgumentException("El nombre no es valido.") : value;
    } = string.Empty;

    public string Apellido {
        get; 
        init => field = !IsNombreValido(value) ? throw new ArgumentException("El apellido no es valido.") : value;
        
    } = string.Empty;

    public int Telefono {
        get;
        init => field = !IsNumaroEspañolValido(value) ? throw new ArgumentException("El teléfono no es válido.") : value;
    }
